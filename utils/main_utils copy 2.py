...
from models.mlpnet import MLPBIG
...
def model_factory(arch,dset_path,pretrained=True):
    if arch == 'mlpnet':
        ...
        criterion = torch.nn.functional.nll_loss
        if pretrained:
            #state_trained = torch.load('checkpoints/mnist_MLPBIG.pth', map_location=torch.device('cpu'))['model_state_dict']
            state_trained = torch.load('checkpoints/mnist_MLPBIG.pth',map_location=torch.device('cpu'))
            model.load_state_dict(state_trained)
            #state_trained = torch.load('checkpoints/mnist_25_epoch_93.97.ckpt',map_location=torch.device('cpu'))['model_state_dict']
            new_state_trained = OrderedDict()
        ...
        #state_trained = torch.load('checkpoints/resnet20_cifar10_70.pth',map_location=torch.device('cpu'))
        new_state_trained = OrderedDict()
        """
        for k in state_trained:
            new_state_trained[k[7:]] = state_trained[k]
        """
        new_state_trained = {k.replace('module.', ''): v for k, v in state_trained.items()}
